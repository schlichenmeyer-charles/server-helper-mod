...
The goal is to provide clear, configurable restart warnings without requiring plugins, command blocks, or client-side mods.
...
The mod reads a schedule from a TOML config file
...
⚠️ The mod does not restart the server process itself.
...
warn_minutes	Minutes before restart to send warnings
...
This mod is intended for server administrators and does not add gameplay content.
